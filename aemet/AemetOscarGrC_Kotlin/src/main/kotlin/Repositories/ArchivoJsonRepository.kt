...
class ArchivoJsonRepository( val programPatch: String): IArchivosAEMET {
    val logger = mu.KotlinLogging.logger {}
    init{
        logger.debug { "INICIANDO ArchivoJsonRepository " }
    }
    override var archivo: RegistroGeneralRDRM = RegistroGeneralRDRM()
    @OptIn(ExperimentalStdlibApi::class)
    override fun leer():RegistroGeneralRDRM?{
        val moshi: Moshi = Moshi.Builder().addLast(KotlinJsonAdapterFactory()).build()
        val jsonAdapter: JsonAdapter<RegistroGeneralRDRM> = moshi.adapter<RegistroGeneralRDRM>()
        archivo= jsonAdapter.fromJson(File(programPatch).inputStream().source().buffer())!!
        return  archivo
    }
    @OptIn(ExperimentalStdlibApi::class)
    override fun escribir() {
        val moshi = Moshi.Builder().addLast(KotlinJsonAdapterFactory()).build()
        val jsonAdapter = moshi.adapter(RegistroGeneralRDRM::class.java)
        val json = jsonAdapter.indent("   ").toJson(archivo)
        println(json)
        File(programPatch).writeText(json)
    }
